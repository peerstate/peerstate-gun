...
import { peerstateGun } from "./peerstateGun";
import Gun from "gun/gun";
import { IGunStatic } from "gun/types/static";

type StateTreeType = any;

interface IGunStatic2 extends IGunStatic {
  on: (name: string, cb: (ctx: any) => void) => void;
}
...
(Gun as IGunStatic2).on(
  "opt",
  peerstateGun(myAuthFilter, myEncryptionFilter, keychain)
);
...
test("some simple gundb stuff", (done) => {
  const gun = Gun<Hello>();
  const hello = gun.get("hello");
  hello.put({ name: "foo bar" });
  hello.get("thing").put({ foo: "baz" });
  // hello.put({ name: "John Smith", thing: { foo: "bar" } });
  gun.get("hello", (res) => {
